...
const AnimatedCarousel = ({
  ...
  const galleryData = useMemo(() => [
    { url: "./images/CALDO/CALDO-1 (PORTADA).jpg", name: "CALDO", year: "2023" },
    { url: "./images/blua_constelaciones_finales.jpg", name: "BLUA", year: "2022" },
    { url: "./images/PLATA/PLATA-2.jpg", name: "PLATA", year: "2021" },
    { url: "./images/CAT-17.jpg", name: "CASA", year: "2020" },
    { url: "./images/L-12.jpg", name: "LUMEN", year: "2024" },
    { url: "./images/NWB&W-09.jpg", name: "NOIR", year: "2023" },
    { url: "./images/S-1.jpg", name: "SOLO", year: "2022" },
    { url: "./images/MDLST/MDLST-1.png", name: "MIDDLEST", year: "2021" },
    { url: "./images/TEO/V1.jpg", name: "TEOREMA", year: "2020" },
    { url: "./images/LENOIR/LENOIR-1.jpg", name: "LENOIR", year: "2023" },
    { url: "./images/KABOA/KABOA-1.jpg", name: "KABOA", year: "2022" },
    { url: "./images/AMOUR/portada.jpg", name: "AMOUR", year: "2021" },
    { url: "./images/MARCOS/MARCOSMUF-5 (PORTADA).jpg", name: "MARCOS", year: "2024" },
    { url: "./images/PASARELA/PASARELA MUF-12(PORTADA).jpg", name: "PASARELA", year: "2023" },
    { url: "./images/IDENTIDAD/IDENTIDAD MUF-1 (PORTADA).jpg", name: "IDENTIDAD", year: "2022" },
  ], []);

  const imageUrls = useMemo(() => galleryData.map(item => item.url), [galleryData]);
  ...
  // --- NEW STATE FOR SELECTED GALLERY INFO ---
  const [selectedGalleryInfo, setSelectedGalleryInfo] = useState(null); // { name: "", year: "" }
  const textRef = useRef(); // Ref for the Text mesh
  ...
  const handleClick = (index) => {
    ...
    if (!(selectedImage.includes(index))) {
      ...
      setSelectedGalleryInfo(galleryData[index]);
  ...
  const animateImageToFront = (index) => {
    ...
    // --- Animate text visibility/position ---
    if (textRef.current) {
      gsap.to(textRef.current.position, {
        y: -3, // Position below the image
        z: 0,
        duration: 1.2,
        ease: "power3.inOut"
      }, 0);
      gsap.to(textRef.current.material, {
        opacity: 1,
        duration: 0.8,
        delay: 0.8 // Appear after image movement starts
      });
      textRef.current.visible = true; // Make visible
    }
  ...
  useFrame(({ camera, scene }) => {
    ...
    if (!animationInProgressRef.current) {
      ...
      // Ensure text also looks at camera
      if (textRef.current && selectedGalleryInfo && isImageUpFront) {
  ...
  const resetImagePositions = () => {
    ...
    timelineRef.current = gsap.timeline({
      onComplete: () => {
        ...
        setSelectedImage([]);
        // --- RESET SELECTED GALLERY INFO ---
        setSelectedGalleryInfo(null); // Clear selected info
    ...
    if (textRef.current) {
      gsap.to(textRef.current.material, {
        ...
        onComplete: () => {
          ...
          // Reset text position to avoid interfering with other animations
          gsap.set(textRef.current.position, { y: -10, z: -10 });
        }
      });
    }
  ...
  }, [refs, isInitializing, isImageUpFront, animationInProgressRef, cameraRef, raycaster, mouse, resetImagePositions]); // Add dependencies
    ...
    <group ref={groupRef}>
      ...
      {selectedGalleryInfo && (
        <>
          <Text
            ref={textRef}
            position={[0, -3, 0]} // Initial position, will be animated
            fontSize={0.6}
            color="black"
            anchorX="center"
            anchorY="middle"
            font="/fonts/HelveticaNeue.ttf" // You'll need to provide a Helvetica font file
            material-toneMapped={false}
            material-transparent={true}
            material-opacity={0} // Start invisible
            visible={false} // Start not visible
          >
            {selectedGalleryInfo.name}
            <meshBasicMaterial attach="material" color="black" transparent opacity={0} />
          </Text>
          <Text
            position={[0, -3.8, 0]} // Slightly below the name
            fontSize={0.4}
            color="black"
            anchorX="center"
            anchorY="middle"
